...
const NewsletterForm: React.FC<NewsletterFormProps> = ({
  ...
  const handleList = (type: 'ordered' | 'unordered') => {
    ...
    if (selectedText) {
      ...
      const newContent =
        content.substring(0, start) + listHtml + content.substring(end);
  ...
  React.useEffect(() => {
    const initializeDragDrop = () => {
      ...
      const handleDragStart = (e: DragEvent) => {
        ...
        if (
          target &&
          (target.classList.contains('newsletter-image') ||
            target.closest('.youtube-embed'))
        ) {
          // Set drag data
          if (e.dataTransfer) {
            const element = target.classList.contains('newsletter-image')
              ? target
              : target.closest('.youtube-embed');
      ...
      const handleDrop = (e: DragEvent) => {
        ...
        if (isImage) {
          ...
          if (imgSrc) {
            elementToRemove = tempDiv.querySelector(
              `img[data-src="${imgSrc}"]`
            );
          }
        } else {
              ...
              <AlertDialogContent className="dark:bg-gray-800 dark:text-white">
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription className="dark:text-gray-300">
                    هل أنت متأكد من رغبتك في إرسال هذه النشرة إلى جميع
                    المشتركين؟ لا يمكن التراجع عن هذا الإجراء.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction onClick={handleSendNewsletter}>
                    إرسال
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
